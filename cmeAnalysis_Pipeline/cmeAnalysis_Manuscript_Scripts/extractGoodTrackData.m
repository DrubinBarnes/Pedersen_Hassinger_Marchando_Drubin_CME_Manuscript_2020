...
goodTrackData = struct('ref_intensity',{},'cor_intensity',{},'ref_intensity_bg_corrected',{},...
    'cor_intensity_bg_corrected',{}, 'ref_tp_first',{}, 'ref_tp_last',{}, 'cor_tp_first',{},...
    'cor_tp_last',{}, 'tp_prev_all',{}, 'tp_post_all',{}, 'ref_coord_extrapol',{}, 'cor_coord_extrapol',{});
        ...
        for foldernum2 = 3:numel(list2)
            if list2(foldernum2).isdir
                ...
                for i = 1:ref_cor_num
                    ...
                    goodTrackData(previous_index+i).ref_intensity = track_data.associated_tracks(ref_track, 1).ref_intensity(1, :);
                    goodTrackData(previous_index+i).cor_intensity = track_data.associated_tracks(ref_track, 1).cor_intensity(1, :);
                    goodTrackData(previous_index+i).ref_intensity_bg_corrected = track_data.associated_tracks(ref_track, 1).ref_intensity_bg_corrected(1, :);
                    goodTrackData(previous_index+i).cor_intensity_bg_corrected = track_data.associated_tracks(ref_track, 1).cor_intensity_bg_corrected(1, :);
                    goodTrackData(previous_index+i).ref_tp_first = track_data.ROI_track_ref(ref_track, 1).ref_tp_first;
                    goodTrackData(previous_index+i).ref_tp_last = track_data.ROI_track_ref(ref_track, 1).ref_tp_last;
                    goodTrackData(previous_index+i).cor_tp_first = track_data.ROI_track_ref(ref_track, 1).cor_tp_first;
                    goodTrackData(previous_index+i).cor_tp_last = track_data.ROI_track_ref(ref_track, 1).cor_tp_last;
                    goodTrackData(previous_index+i).tp_prev_all = track_data.ROI_track_ref(ref_track, 1).tp_prev_all;
                    goodTrackData(previous_index+i).tp_post_all = track_data.ROI_track_ref(ref_track, 1).tp_post_all;
                    goodTrackData(previous_index+i).ref_coord = track_data.ROI_track_ref(ref_track, 1).center_coord;
                    goodTrackData(previous_index+i).cor_coord = track_data.ROI_track_cor(cor_track, 1).center_coord;
                    goodTrackData(previous_index+i).ref_coord_extrapol = track_data.ROI_track_ref(ref_track, 1).ref_coord_extrapol;
                    goodTrackData(previous_index+i).cor_coord_extrapol = track_data.ROI_track_ref(ref_track, 1).cor_coord_extrapol;
...
save(fullfile(data_folder, 'goodTrackData.mat'), 'lifetime_associated', 'track_stat_asso','goodTrackData')
